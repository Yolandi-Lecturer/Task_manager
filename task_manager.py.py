...
# Variables to be used during the program.
current_path = os.path.dirname(__file__)
delay_menu = 1.5
delay_login_exit = 1
first_login = True

# funtionality to login a user
def login():
    ...
    while not user_pass:
        ...
        # Print the welcome information to the screen and add a delay to make the program feel more realistic
        else:
            print("Logging in...")
            time.sleep(delay_login_exit)
            print("\nLogin successful, welcome to the task manager!")
            ...
            for y in range(0, len(tasks)):
                ...
                if users[x][0] == tasks[y][0]:
                    ...
                    if tasks[y][5] == "Yes":
                        ...
                        num_incomplete += 1
                        task_date = datetime.strptime(tasks[y][4], '%Y-%m-%d')
                        if datetime.date(datetime.now()) > task_date.date():
                            num_inc_overdue += 1
